...
export const actions = {
    async nuxtServerInit({ dispatch }, { $axios }) {
        ...
        const cartesPostales = await $axios.$get('https://vjs2pfjl.api.sanity.io/v2022-03-07/data/query/production?query=*[_type == "cartesPostales"]{name, afficherNom, immanquables,"imageId":image{asset,alt}, stripeUrl}')

        const imageSite = await $axios.$get('https://vjs2pfjl.api.sanity.io/v2022-03-07/data/query/production?query=*[_type == "imageSite"]{name, afficherNom, "imageId":image{asset,alt}, stripeUrl, typesCartes}')
